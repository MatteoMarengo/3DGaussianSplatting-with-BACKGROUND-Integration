...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # If we have a connection, receive the data
        while network_gui.conn is not None:
            try:
                net_image_bytes = None
                custom_cam, do_training, pipe.convert_SHs_python, pipe.compute_cov3D_python, keep_alive, scaling_modifer = network_gui.receive()
                # If we have a custom camera, render the image
                if custom_cam is not None:
                    net_image = render(custom_cam, gaussians, pipe, bg_image, scaling_modifer)["render"]
        ...
        if iteration % 500 == 0:
            ...
            # plt.imsave(f"/home/mmarengo/workspace/3DGS-Matteo-Exp/data-3DGS/NuScenes-Keyframes/saved_iterations/background{iteration}.png", image_to_save.detach().cpu().numpy())
        ...
        with torch.no_grad():
            ...
            ####################################################################
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, bg_image))
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--test_iterations", nargs="+", type=int, default=[100, 200])
    # parser.add_argument("--save_iterations", nargs="+", type=int, default=[300])
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[1_000, 7_000, 10_000, 20_000,30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[1_000, 7_000, 10_000, 20_000,30_000])
    # parser.add_argument("--test_iterations", nargs="+", type=int, default=[1_000, 2_000, 3_000])
    # parser.add_argument("--save_iterations", nargs="+", type=int, default=[1_000, 2_000, 3_000])
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[])
    parser.add_argument("--start_checkpoint", type=str, default=None)
    #parser.add_argument("--background_image_path", type=str, required=True, help="Path to the background image")
    ...
    #training(args.background_image_path,lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from)
